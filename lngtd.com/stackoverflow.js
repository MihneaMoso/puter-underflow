...
lPS();}
const sc=[];sc.push({"script_path":"stackoverflow/default.1772208603.js","pct":100})
window.__lngtdReload=function(){if(fgp("lngtd_version")){let scriptPath=`stackoverflow/${fgp("lngtd_version")}.js`;lw(scriptPath);}else{const exp=sc.flatMap(co=>Array(co.pct).fill(co));const w=exp[Math.floor(Math.random()*exp.length)];lw(w.script_path);}};function tryInject(){window.__lngtdReload();}
tryInject();}());